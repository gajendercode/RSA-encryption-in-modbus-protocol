...
class Connection:
    ...
    async def _write(self, data):
        if type(self).__name__ =='Client':              #if the _write function is called by client
            self.log.info("Data which is to be sent to Client before encryption : %r", data)
            # encrypt the message with the encryption key
            encrypt_data = self.fernet.encrypt(data)            
            self.log.info("Data is encrypted, now sending to client : %r", encrypt_data)
            self.writer.write(encrypt_data)
        else:                                           #if the _write function is called by modbus device
            self.log.info("Data sending to modbus device : %r", data)
    ...
    async def _read(self):
        ...
        if type(self).__name__ =='Client':   #if the _read function is called by client
            #reading the 1024 size byte stream from the client
            Encrypt_data = await self.reader.read(1024)
            self.log.info("Encrypted data received from client : %r", Encrypt_data)
            #decrypting the message with fernet(encryption key)
            reply = self.fernet.decrypt(Encrypt_data)
        else:	                                #if the _read function is called by modbus device
            header = await self.reader.readexactly(6)
            size = int.from_bytes(header[4:], "big")
            reply = header + await self.reader.readexactly(size)
        self.log.info("Data received from modbus device : %r", reply)
        return reply
...
class ModBus(Connection):
    ...
    async def handle_client(self, reader, writer):
        async with Client(reader, writer) as client:
            '''
            ----KEYS EXCHANGE BETWEEN CLIENT AND MODBUS PROXY-----
            '''
			#saving the public key in PEM format
            self.log.info(publicKey.save_pkcs1())
            #sending public key of modbus proxy server to client
            client.writer.write(publicKey.save_pkcs1())
            #recieving the encrypted encryption key 
            enc_encryption_Key= await client.reader.read(1024)
            #decrypting the encryption key 
            encryption_Key = rsa.decrypt(enc_encryption_Key, privateKey)
            self.log.info("Encryption key is : " + str(encryption_Key))
            #creating a fernet instance of the encryption key which will be used for encrypt/decrypt the messages
            client.fernet = Fernet(encryption_Key)
